...
Hiring artifact for **Luke Kimball** — four working demos, one repo. Synthetic product is **RouteKit**, a fake OpenAI-compatible LLM gateway. No real customers, no PII, no production metrics.

The last 20% is the point: structured actions, deterministic guardrails, evals that do not brittle-match full replies, a launch gate you can run without an API key — and, in Caesar, traces plus concession rules rather than a pretty wrapper.
...
| `caesar/` | Where models go when you let them go at each other | Two OpenRouter models debate in a text chat. Caesar (a third model, overlooker — not a debater) scores each round and can call the match. Ends on a real concession (regex + Caesar agree) or max rounds. Dense JSON traces, not a dashboard. |
...
python -m caesar.harness          # MODEL_A / MODEL_B / CAESAR_MODEL
```
...
Caesar traces: `results/caesar/<id>.json` (and `matches.jsonl`). Replay without a key:

```bash
python -m caesar.harness
# then open caesar/chat.html in a browser and choose results/caesar/routing-vs-direct.json
```

The last 20% of Caesar is the concession rule and the trace schema: Caesar cannot award a concession the speaker did not say, and every turn records model slug, latency, tokens, cost, chars, tool-search, claims, grounding, points.
...
- **Caesar is a transcript.** Two speakers, one judge. A concession is a speech act in the turn text, not a vibe Caesar hallucinates. Jailbreak topics are judged, not executed.
- **Offline CI.** Two stub “models” (`stub/capable`, `stub/sloppy`) so every harness works without a key. Caesar adds `stub/capable-a`, `stub/capable-b`, `stub/caesar`.
...
caesar/    debate.py caesar.py trace.py cases.jsonl harness.py chat.html
tests/
```
